// ─────────────────────────────────────────────────────────────
// Datos del Asistente Neodomus — PORT LITERAL de la WEB
// (fe/src/data/botData.ts). El asistente web es 100% frontend:
// NO existe endpoint de chatbot en el backend, así que el móvil
// replica exactamente la misma lógica y textos.
// ─────────────────────────────────────────────────────────────
...
export const BOT_SUGERENCIAS: string[] = [
  "¿Cómo hago un pedido?",
...
/** Mismo matching que la WEB: lowercase + includes, primera FAQ con keyword. */
export function responderBot(mensaje: string): string {
  const texto = mensaje.toLowerCase();
  for (const faq of BOT_FAQS) {
    if (faq.keywords.some((keyword) => texto.includes(keyword))) {
