"""
Resetea las contraseñas de TODOS los CLIENTES con el patrón:
...
ejecutarse varias veces con el mismo resultado. No afecta a los empleados.

Uso (dentro del contenedor api):

    docker exec -it neodomus_api uv run python /app/scripts/reset_passwords_clientes.py
"""
...
def run(mostrar_claves: bool = False) -> int:
    ...
    with SessionLocal() as db:
        for registro in db.execute(select(Cliente)).scalars():
            pwd = password_de(registro.first_name, registro.last_name)
            registro.password_hash = hash_password(pwd)
            actualizados += 1
            print(
                f"[actualizado] clientes   {registro.email} -> {pwd if mostrar_claves else ''}"
            )
        db.commit()
    return actualizados
...
if __name__ == "__main__":
    ...
    print(f"\nContraseñas de clientes actualizadas: {total}")
